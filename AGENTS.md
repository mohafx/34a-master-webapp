...
| [04-edge-functions.md](docs/agents/04-edge-functions.md) | Katalog der Supabase Edge Functions |
...
- ✅ **Prod-Launch mündliche Prüfung (2026-06-20):** Für alle Nutzer freigeschaltet (`c06097b`), Tickets live, PostHog-Funnel-Tracking aktiv (`548ab0e`), Redeploy mit PostHog-Host-Fix (`fffe945`).
- ✅ **Paywall-Redesign (2026-06-20):** Exam-Intros, Modal-Portal-Fix, Paywall-Conversion-Optimierung (`33df7e2`), Dashboard-Messaging verfeinert (`29adec0`).
- ✅ **E-Mail-Kampagne gebaut (2026-06-20):** Kampagne für mündliche Prüfung mit Abmelde-System, Daily Limits und Duplikat-Prävention (`7bb5d5f`, `f059e97`). Dokumentiert in `docs/agents/10-email-kampagnen.md`.
- ✅ **StGB-Fragen & Lektionsabschluss korrigiert (2026-06-20):** Drei gemeldete Antwortschlüssel in `questions` behoben (Wodka = C, Schlagstock/Platzwunde = B, Fahrradschloss = B,D), Cache-Version auf `v3` erhöht und Auto-Abschluss der letzten Lektionsfrage entfernt, damit die Erklärung vor dem Abschluss sichtbar bleibt. Migration `20260620182122` ist remote angewendet.
- ✅ **Sentry-Produktionsfehler reduziert (2026-06-20):** Chunk-/MIME-Deployment-Fehler werden in `ErrorBoundary` erkannt und pro Signatur einmal automatisch neu geladen, frühe `AppContent`-Rückgaben liegen im `AppContext.Provider`, fehlender `supabase`-Import in `Statistics.tsx` behoben. Build und mündliche Prüfungs-Tests grün; repo-weiter `tsc --noEmit` hat bekannte Altfehler außerhalb dieses Fixes.
- ✅ **Mobiler Cold-Start optimiert (2026-06-21):** Paywall/Stripe-Code wird lazy geladen; `App.tsx` nutzt gezielte `lucide-react`-Imports. `DataCacheProvider` lädt zuerst Module, Fragen-Preview und Lernkarten-Preview, gibt danach Dashboard/Listen frei und lädt vollständige Fragen/Lernkarten im Hintergrund. Cache-Version `v4` mit `fullDataReady`; `QuestionView` startet nicht mit Preview-Fragen ohne Antworten, sondern fällt auf die bestehende Supabase-Abfrage zurück. Premium-/Paywall-/Stripe-/Entitlement-Logik unverändert. Build und Regressionstests grün; iPhone-Preview geprüft.
- ✅ **Premium-Freischaltung stabilisiert (2026-06-21):** Nach Kundenbeschwerde serverseitige Premium-Quelle `entitlement-status` ergänzt, `PaymentSuccess` und `SubscriptionContext` gegen mobilen Session-/State-Delay gehärtet, `payment_audit_events` eingeführt, Refunds auf `refunded` mit sofortigem Periodenende korrigiert. Supabase-Migration und Edge Functions (`entitlement-status`, `verify-checkout`, `stripe-webhook`) produktiv deployed; `npm run test:premium-entitlement` und Build grün.
- ✅ **Mündliche Prüfung: Bonus-Tickets (2026-06-22):** `oral_exam_ticket_grants` ergänzt; aktive Grants erhöhen serverseitig das Free-/Premium-Ticketlimit, verbraucht wird weiterhin ausschließlich über `oral_exam_sessions.connected_at`. UI-Fehlermeldungen nutzen das dynamische Limit. `npm run test:oral-exam-entitlement` grün.
- ✅ **Quiz-Erklärbilder Self-Service (2026-06-22):** Fest verknüpfte Erklärbilder zurückgesetzt und UI-Button „Erklärbild erstellen“ eingeführt. Edge Function `generate-question-image` erzeugt pro Frage genau ein gecachtes PNG in Supabase Storage `question-explanations`; vorhandene Bilder blocken Neugenerierung für normale Nutzer. Admins sehen zusätzlich „Bild löschen“ und „Bild regenerieren“ (serverseitig per Admin-JWT geschützt). Aktueller Prompt: `gpt-image-2`, `1024x1024`, `medium`, mobile-first, Blau exakt `#4E81EE`, vier Referenzbilder unter `public/question-explanations/*.PNG` bzw. Storage `_references/`. Remote deployed; Build und `npm run test:question-images` grün.
