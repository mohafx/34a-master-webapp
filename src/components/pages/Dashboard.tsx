    ...
    return (
        <div className="bg-[#F2F4F7] dark:bg-slate-950 text-slate-900 dark:text-white min-h-screen font-sans overflow-x-hidden">
            <div className="max-w-4xl mx-auto relative w-full pb-20 pt-4 text-left">
                <div className="px-4">
                    <header className="bg-[#3B68FF] rounded-[24px] px-5 py-4 sm:px-7 sm:py-6 mb-6 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] text-white relative overflow-hidden text-left">
                        <div className="absolute -right-10 -top-10 w-40 h-40 bg-white opacity-10 rounded-full blur-2xl"></div>
                        <div className="flex justify-between items-center mb-8 relative z-10">
                            <div className="flex flex-col">
                                <h1 className="text-2xl font-black tracking-tighter text-white">34a Master</h1>
                                {user && (
                                    <div className="mt-1">
                                        <span className="text-white/80 text-sm font-bold block">Hallo, {userName}! 👋</span>
                                        {language === 'DE_AR' && (
                                            <span className="text-white/70 text-sm font-medium block mt-0.5" dir="rtl" style={{ textAlign: 'left' }}>مرحباً، {userName}! 👋</span>
                                        )}
                                    </div>
                                )}
                            </div>
                            <div className="flex gap-3">
                                {showLanguageToggle && (
                                    <button onClick={(e) => { e.stopPropagation(); toggleLanguage(); }} aria-label="Language" className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm transition-transform active:scale-95">
                                        <span className="material-icons text-[20px]">translate</span>
                                    </button>
                                )}
                                <button onClick={(e) => { e.stopPropagation(); navigate('/profile'); }} aria-label="Settings" className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm transition-transform active:scale-95">
