...
import { Search, X, ChevronDown, ChevronUp, Pencil, Save, XCircle, Plus, Trash2, CheckCircle, Circle, Filter } from 'lucide-react';
...
interface AdminWrittenQuestionsProps {
...
export default function AdminWrittenQuestions({ questions, topics, onSave, saving }: AdminWrittenQuestionsProps) {
    ...
    const filtered = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            result = result.filter(q =>
                q.question_text_de?.toLowerCase().includes(query) ||
                q.question_text_ar?.toLowerCase().includes(query) ||
                q.answer_a_de?.toLowerCase().includes(query) ||
                q.answer_b_de?.toLowerCase().includes(query)
            );
        }
        return result;
    }, [questions, searchQuery, filterTopic]);
    ...
    const handleSave = async () => {
        ...
        const updates: any = {
            question_text_de: editData.question_text_de,
            question_text_ar: editData.question_text_ar || null,
    ...
    const getQuestionType = (q: WrittenQuestion) => {
        return q.correct_answer.includes(',') ? 'Multiple Choice' : 'Single Choice';
    };

    const getAnswerCount = (q: any) => {
        let count = 0;
        for (const letter of ANSWER_LETTERS) {
            if (q[`answer_${letter.toLowerCase()}_de`]) count++;
        }
        return count;
    };
    ...
    const toggleCorrectAnswer = (letter: string) => {
        ...
        const isMulti = current.length > 1 || editData.correct_answer?.includes(',');

        if (current.includes(letter)) {
            // Remove
            const newAnswers = current.filter((a: string) => a !== letter);
            setEditData({ ...editData, correct_answer: newAnswers.join(',') || '' });
        } else {
            // Add
        ...
        <div>
            ...
            <div className="mb-4 space-y-3">
                <div className="relative">
                    ...
                    <input
                        ...
                        placeholder="Prüfungsfragen durchsuchen..."
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all ${showFilters || filterTopic
                    ...
                    >
                        <Filter size={14} />
                        Filter
                        {filterTopic && <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />}
                    </button>
                ...
                {showFilters && (
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-200 dark:border-slate-700">
                        <label className="text-xs font-bold text-slate-600 dark:text-slate-400 mb-1.5 block">Thema</label>
                        <select
                            value={filterTopic}
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-slate-900 dark:text-white leading-snug">
                                            {data?.question_text_de || q.question_text_de}
                                        </p>
                                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${q.correct_answer.includes(',')
                                    ...
                                    {isEditing ? (
                                        <div className="p-4 space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <textarea
                                                    value={editData.question_text_de}
                                                    onChange={e => setEditData({ ...editData, question_text_de: e.target.value })}
                                                    rows={3}
                                            ...
                                            <div>
                                                ...
                                                <textarea
                                                    value={editData.question_text_ar || ''}
                                                    onChange={e => setEditData({ ...editData, question_text_ar: e.target.value })}
                                                    rows={2}
                                                    dir="rtl"
                                                    className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 resize-none"
                                                />
                                            ...
                                            {/* Topic */}
                                            <div>
                                                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1 block">Thema</label>
                                                <select
                                        ...
                                        <div className="p-4 space-y-3">
                                            {q.question_text_ar && (
                                                <div>
                                                    ...
                                                    <p className="text-sm text-slate-700 dark:text-slate-300 mt-0.5" dir="rtl">{q.question_text_ar}</p>
                                                </div>
                                            )}

                                            <div className="text-[11px]">
                                                <span className="font-bold text-slate-400 dark:text-slate-500">Thema: </span>
                                                <span className="font-medium text-slate-600 dark:text-slate-300">{q.topic}</span>
                                            </div>

                                            <div className="space-y-1.5">
                                                {ANSWER_LETTERS.map(letter => {
                                                    const key = letter.toLowerCase();
                                                    const textDE = q[`answer_${key}_de` as keyof WrittenQuestion] as string | null;
            ...
            {filtered.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-sm text-slate-400 font-medium">Keine Prüfungsfragen gefunden</p>
                </div>
            )}
        </div>
    );
