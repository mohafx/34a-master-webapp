...
export const db = {
    ...
    async completeWrittenExamSession(sessionId: string, score: number, answers?: Record<string, string>) {
        const { error } = await supabase
            .from('written_exam_sessions')
            .update({
                completed_at: new Date().toISOString(),
    ...
    _mapWrittenExamSession(s: any) {
        return {
            ...
            examType: (s.total_questions === 16 ? 'mini' : 'full') as 'mini' | 'full'
        };
    },

    async getActiveWrittenExamSession(userId: string, examType: 'full' | 'mini' = 'full') {
        const totalQuestions = examType === 'mini' ? 16 : 82;
        const { data, error } = await supabase
            .from('written_exam_sessions')
            .select('*')
            .eq('user_id', userId)
            .eq('total_questions', totalQuestions)
        ...
        if (error) throw error;
        return data ? this._mapWrittenExamSession(data) : null;
    },

    async getWrittenExamSessions(userId: string, options?: { completedOnly?: boolean; examType?: 'full' | 'mini'; limit?: number }) {
        let query = supabase
            .from('written_exam_sessions')
            .select('*')
            .eq('user_id', userId);
        ...
        if (options?.examType) {
            query = query.eq('total_questions', options.examType === 'mini' ? 16 : 82);
        }
        ...
        return (data || []).map((s: any) => this._mapWrittenExamSession(s));
    },
